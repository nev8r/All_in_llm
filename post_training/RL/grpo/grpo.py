...
class GRPOConfig:
    ...
    output_dir = "/root/All_in_llm/post_training/RL/grpo/output"
    ...
    clip_eps = 0.2 # cliorange
    gradient_accumulation_steps = 4 # 梯度累加
    num_iterations = 3 # 采样一次样本训练模型轮数
    batch_size = 3
...
class GRPOTrainer:
    ...
    def __init__(self,
        model = None,
        reward_funcs: Union[List[str], List[Callable]] = None, # 奖励计算函数
        args = None,
    ...
    def generate_samples(self,inputs):
        ...
        max_len = self.args.max_generate_length + self.args.max_prompt_length
        for prompt,answer in zip(prompts,answers):
            ...
            if prompt_response_ids.size(1) >= max_len:
                ...
                prompt_response_ids = torch.cat([prompt_response_ids, torch.full((prompt_response_ids.size(0), max_len - prompt_response_ids.size(1)), fill_value=self.tokenizer.pad_token_id, device=prompt_response_ids.device)], dim=1)
    ...
    def compute_loss(
        ...
        if self.args.beta != 0.0:
            ...
            # k3: log_ratio.exp() - 1 - log_ratio k3 KL
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained('/root/All_in_llm/models/Qwen2.5-1.5B-Instruct')
    model = AutoModelForCausalLM.from_pretrained('/root/All_in_llm/models/Qwen2.5-1.5B-Instruct')
        
    prompts_dataset = GSM8KDataset('/root/All_in_llm/datasets/gsm8k-grpo-format', tokenizer)
